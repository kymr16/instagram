...
//  Instagram
//
//  Created by 吉田 玲子 on 2020/07/04.
...
class HomeViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate  {
   ...
   @objc func handleLikeButton(_ sender: UIButton, forEvent event: UIEvent) {
     ...
     // コメントを入力するボタンがタップされた時に呼ばれるメソッド
    @objc func handleCommentButton(_ sender: UIButton, forEvent event: UIEvent) {
        print("DEBUG_PRINT: コメントを入力するボタンがタップされました。")
        ...
        //コメント投稿画面を表示
        let commentPostViewController = self.storyboard?.instantiateViewController(withIdentifier: "CommentPost") as! CommentPostViewController
        commentPostViewController.parentPostID = postID
        commentPostViewController.parentImageRef = imageRef
        commentPostViewController.parentCaption = "\(postData.name!) : \(postData.caption!)"
